...
*This is an alternate framing of the pattern documented in README.md. It reframes the pitch around agent deployment on existing infrastructure, adds the dispatch room analogy, a workflow diagram, and answers to a few common framing questions. README.md remains the canonical reference for setup mechanics.*
...
This pattern is a derivative of the Interpretable Context Methodology (ICM), which treats folder structure as agent architecture: a stage contract format (Inputs, Process, Outputs), canonical sources for shared facts, and lightweight checks before an action fires. readme-runtime narrows that methodology to a single, quickly deployable case: one folder, one README, one REST API, running on infrastructure a Claude Code user already has.

The underlying argument, laid out by the ICM creator in a walkthrough of file trees as agent architecture ([source](https://www.youtube.com/shorts/tbVtt2-qUJo)), is that most agent frameworks (LangChain, the Anthropic Agent SDK used as a router, Semantic Kernel, and others in that category) solve a routing problem a file tree already solves on its own. A workflow is a folder. A task inside that workflow is a subfolder. Instructions, tools, and data for that task live underneath it as further subfolders or files. A single coding agent already reads instructions, calls tools including MCP servers for outside data, spawns subagents for other tasks, and creates new structure inside that folder, with no separate orchestration layer written in Python or any other language. readme-runtime sits at the flat end of that same claim: one folder holds one README, and inside that single file the instructions, tool surface, and guardrails appear as sections rather than as separate subfolders, pointed at one REST API.
...
- **Existing infrastructure.** Anyone with Claude Code already has the runtime. A cheap always-on host and a persistent session are the only additions, described in the setup section below.
